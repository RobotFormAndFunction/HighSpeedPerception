...
const int IMAGE_WIDTH = 176; // set the camera properties to this size in the configure file
const int IMAGE_HEIGHT = 144; // set the camera properties to this size
...
void setup() {
  ...
  config.frame_size = FRAMESIZE_QCIF; //changed to qvga for (320 x 240)
  config.pixel_format = PIXFORMAT_GRAYSCALE; // changed to grayscale
  config.grab_mode = CAMERA_GRAB_WHEN_EMPTY;
  config.fb_location = CAMERA_FB_IN_PSRAM;
  config.jpeg_quality = 2; //this can be adjusted to create lower or higher quality images
  config.fb_count = 1;
